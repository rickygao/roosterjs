 ...
 */
export type PluginKey<TPlugins> = keyof TPlugins;

/**
 * Names of the core plugins that have plugin state
 */
export type KeyOfStatePlugin<
    Key extends PluginKey<TPlugins>,
    TPlugins
> = TPlugins[Key] extends PluginWithState<infer U> ? Key : never;
...
export type TypeOfStatePlugin<
    Key extends PluginKey<TPlugins>,
    TPlugins
> = TPlugins[Key] extends PluginWithState<infer U> ? U : never;

/**
 * All names of plugins with plugin state
 */
export type StatePluginKeys<Key extends PluginKey<TPlugins>, TPlugins> = {
    [P in Key]: KeyOfStatePlugin<P, TPlugins>;
}[Key];

/**
 * A type map from name of plugin with state to its plugin type
 */
export type GenericPluginState<Key extends PluginKey<TPlugins>, TPlugins> = {
    [P in StatePluginKeys<Key, TPlugins>]: TypeOfStatePlugin<P, TPlugins>;
};

/**
 * Auto-calculated State object type for plugin with states
 */
export type PluginState = GenericPluginState<PluginKey<CorePlugins>, CorePlugins>;
