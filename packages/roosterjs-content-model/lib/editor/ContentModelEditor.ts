...
export default class ContentModelEditor extends Editor implements IContentModelEditor {
    ...
    setContentModel(model: ContentModelDocument, option?: ModelToDomOption) {
        ...
        const newOption: ModelToDomOption = {
            ...(option || {}),
            existingRootNode: this.isFeatureEnabled(ExperimentalFeatures.ReusableContentModel)
                ? contentDiv
                : undefined,
        };
        const [root, range, entityPairs] = contentModelToDom(
            this.getDocument(),
            model,
        ...
        if (root == contentDiv) {
            this.select(range);
        } else if (range?.type == SelectionRangeTypes.Normal) {
            ...
            restoreContentWithEntityPlaceholder(root, core.contentDiv, entityPairs);
